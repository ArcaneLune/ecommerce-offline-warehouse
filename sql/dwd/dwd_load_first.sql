-- ============================================================
-- DWD 层首日装载（全量数据，动态分区）
-- 执行：hive --hivevar dt=YYYY-mm-dd -f dwd_load_first.sql
-- ============================================================
...
SELECT CAST(id AS STRING), CAST(user_id AS STRING), CAST(sku_id AS STRING),
    ...
    CAST(sku_num AS BIGINT), SUBSTRING(create_time,1,10)
...
SELECT CAST(od.id AS STRING), CAST(od.order_id AS STRING), CAST(oi.user_id AS STRING),
    ...
    od.split_total_amount, SUBSTRING(od.create_time,1,10)
...
SELECT CAST(od.id AS STRING), CAST(od.order_id AS STRING), CAST(oi.user_id AS STRING),
    ...
    COALESCE(od.split_activity_amount,CAST(0.0 AS DECIMAL(16,2))),
    COALESCE(od.split_coupon_amount,CAST(0.0 AS DECIMAL(16,2))),
    od.split_total_amount, SUBSTRING(pi.callback_time,1,10)
FROM ods_order_detail_inc od
...
SELECT CAST(id AS STRING), CAST(coupon_id AS STRING), CAST(user_id AS STRING),
    ...
    SUBSTRING(used_time,1,10)
FROM ods_coupon_use_inc WHERE used_time IS NOT NULL;

-- 9.7 收藏
INSERT OVERWRITE TABLE dwd_interaction_favor_add_inc PARTITION (dt)
SELECT CAST(id AS STRING), CAST(user_id AS STRING), CAST(sku_id AS STRING),
    date_format(create_time,'yyyy-MM-dd'), create_time, SUBSTRING(create_time,1,10)
